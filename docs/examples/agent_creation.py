...
# Constants for example configuration
...
class ZetaAIClient:
    ...
    async def create_agent(self, name: str, description: str, config: AgentConfig) -> Agent:
...
async def example_basic_agent_creation() -> Agent:
    """Example: Create a basic AI assistant agent."""
    print("🤖 Creating a basic AI assistant...")

    client = ZetaAIClient(BASE_URL, API_KEY_PLACEHOLDER)
...
async def example_specialized_agents() -> list[Agent]:
    ...
    code_agent = await client.create_agent(
        name="Code Assistant",
    ...
    creative_agent = await client.create_agent(
    ...
    analysis_config = AgentConfig(
        ...
        max_tokens=2000,
        system_prompt=(
            """You are a data analysis expert. You help with:
    ...
    analysis_agent = await client.create_agent(
        name="Data Analyst",
...
async def example_agent_management() -> Agent:
    """Example: Manage agent lifecycle and configuration."""
    print("🛠️ Demonstrating agent management...")
    ...
    agent = agents[0]
    detailed_agent = await client.get_agent(agent.id)
    ...
    # Update agent configuration
    updated_agent = await client.update_agent(
        agent.id,
...
async def example_agent_with_memory() -> Agent:
    ...
    memory_config = AgentConfig(
        ...
        max_tokens=2000,
        system_prompt=(
            """You are an AI assistant with excellent memory capabilities.
...
async def example_multi_agent_coordination() -> list[Agent]:
    ...
    research_agent = await client.create_agent(
    ...
    planning_config = AgentConfig(
        ...
        max_tokens=2000,
        system_prompt=(
            """You are a project planning specialist. Your role is to:
    ...
    planning_agent = await client.create_agent(
    ...
    execution_config = AgentConfig(
        ...
        max_tokens=2000,
        system_prompt=(
            """You are an execution specialist. Your role is to:
    ...
    execution_agent = await client.create_agent(
...
async def main() -> None:
    ...
    try:
        ...
        await example_basic_agent_creation()
        print()
        ...
        print(f"📈 Total agents created: {1 + len(specialized_agents) + 1 + len(team_agents)}")
