...
try:
    ...
    from .memory_storage import MFAMemoryStorage, VerificationMemoryStorage
    ...
    from .metrics import MFAMetrics, initialize_metrics, AuthMetrics
...
def make_verification_storage(backend: Literal["memory", "redis"] = "memory", **kwargs) -> VerificationStorage:
    ...
    if backend == "memory":
        return VerificationMemoryStorage()
    if backend == "redis":
        try:
