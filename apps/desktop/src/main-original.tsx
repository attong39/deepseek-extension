...
import { loadPluginManifests } from "@/services/plugin.manifest";
import AI from "AI";
...
function AppBoot() {
  ...
  // Load plugin manifests on startup (enterprise safety)
  useEffect(() => {
    try {
      const manifests = loadPluginManifests();
      console.log(`✅ Loaded ${manifests.length} plugin manifests safely`);
      // Store manifests in app state or dispatch to global store
    } catch (error: any) {
      console.warn('⚠️ Plugin manifest loading failed:', error?.message || 'Unknown error');
      // Non-blocking: app continues without plugins
    }
  }, []);
  return (
