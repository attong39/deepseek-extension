...
DISK_IO_BYTES = Counter("disk_io_bytes_total", "Disk I/O bytes", ["operation"])  # read/write
DISK_IO_OPERATIONS = Counter("disk_io_operations_total", "Disk I/O operations", ["operation"])
DISK_IO_TIME = Histogram("disk_io_seconds", "Disk I/O time", ["operation"])

# Network I/O (optional psutil)
NETWORK_BYTES = Counter("network_bytes_total", "Network bytes", ["direction"])  # sent/received
NETWORK_PACKETS = Counter("network_packets_total", "Network packets", ["direction"])
NETWORK_CONNECTIONS = Gauge("network_connections_active", "Active network connections", ["type"])

# Database connections (if available)
DB_CONNECTIONS_ACTIVE = Gauge("db_connections_active", "Active database connections")
DB_CONNECTIONS_IDLE = Gauge("db_connections_idle", "Idle database connections")
DB_QUERY_DURATION = Histogram("db_query_seconds", "Database query duration", ["query_type"])
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        if net_io:
                            NETWORK_BYTES.labels(direction="sent")._value._value = net_io.bytes_sent
                    ...
                    try:
                        ...
                        tcp_count = len([c for c in connections if c.type.name == "SOCK_STREAM"])
                        udp_count = len([c for c in connections if c.type.name == "SOCK_DGRAM"])
                    ...
                    try:
                        ...
                        for gen, (current, last) in enumerate(
                            ...
                            if current != last:
                                ENHANCED_GC_COLLECTIONS_TOTAL.labels(generation=str(gen)).inc(
                                    current - last
                                )
                    ...
                    try:
                        from zeta_vn.perf.config import get_runtime

                        runtime = get_runtime()
                        # Using existing metrics from base instrumentation
                        from zeta_vn.perf.instrumentation import RUNTIME_STATE_GAUGE
    ...
    async def enhanced_metrics_middleware(request: Request, call_next) -> Response:
        ...
        try:
            ...
            try:
                ...
                HTTP_REQUEST_SIZE_ENHANCED.labels(method=method, route=route_path).observe(
                    request_size
                )
...
def record_gc_time(generation: int, duration_seconds: float) -> None:
    ...
    ENHANCED_GC_COLLECTION_TIME.labels(generation=str(generation)).observe(duration_seconds)
```